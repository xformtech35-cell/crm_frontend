...
export default function LeadForm({ initial, loading, onSubmit, quotation, onUploadFiles }) {
  ...
  async function uploadFiles(files) {
    ...
    // Only take first 4 files
    fileList.slice(0, 4).forEach((file, index) => {
      fileMap[slots[index]] = file;
    });
    ...
    try {
      ...
      if (onUploadFiles) {
        await onUploadFiles(fileMap);
      } else {
        // Fallback: just update the form
        console.log('Files to upload:', fileMap);
      }
      ...
      setUploadProgress(0);
      if (window.showToast) {
        window.showToast('error', 'Failed to upload files');
      }
    }
  }
